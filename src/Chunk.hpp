...
class Chunk
{
	...
	static constexpr const float _tex_atlas_width = 8.0f;
	...
	// dot(height, width) == 0.
	void _addRectangle(glm::vec3 center, glm::vec3 height, glm::vec3 width, unsigned tex_num);
...
public:
	...
	static void Render(const CameraData&, const std::vector<Chunk*>&);
};
